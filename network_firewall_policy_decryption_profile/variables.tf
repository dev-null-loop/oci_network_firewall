variable "are_certificate_extensions_restricted" {
  description = "(Applicable when type=SSL_FORWARD_PROXY) (Updatable) Whether to block sessions if the server's certificate uses extensions other than key usage and/or extended key usage."
  type        = bool
  default     = null
}
...
variable "is_auto_include_alt_name" {
  description = "(Applicable when type=SSL_FORWARD_PROXY) (Updatable) Whether to automatically append SAN to impersonating certificate if server certificate is missing SAN."
  type        = bool
  default     = null
}

variable "is_expired_certificate_blocked" {
  description = "(Applicable when type=SSL_FORWARD_PROXY) (Updatable) Whether to block sessions if server's certificate is expired."
  type        = bool
  default     = null
}
